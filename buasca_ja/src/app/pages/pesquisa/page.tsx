...
export default function PesquisaPage() {
  ...
  const handleSearch = () => {
    const query = new URLSearchParams(form as any).toString(); // serializa o form
  ...
  Busca Já
      ...
      <section className="px-6 py-8 mx-4 mt-6 text-white rounded-3xl overflow-hidden relative">
        ...
        <div className="relative z-10 bg-transparent p-8 rounded-3xl shadow-md">
          <h1 className="text-2xl font-semibold text-center">Foco em tranquilidade para o cliente</h1>
          <p className="text-center mt-2">&quot;Deixe a carga com a gente. Você só se preocupa com o destino.&quot;</p>
  ...
  <div className="flex flex-col px-4 border-r last:border-none">
    <span className="text-sm font-semibold text-gray-700">Cidade/Estado</span>
    <input placeholder="Para onde será o frete?" className="outline-none text-sm text-gray-500 placeholder-gray-400 bg-transparent" value={form.cidade} onChange={e => handleChange('cidade', e.target.value)} />
  </div>
  <div className="flex flex-col px-4 border-r last:border-none">
    <span className="text-sm font-semibold text-gray-700">Endereço</span>
    <input placeholder="Nome da rua" className="outline-none text-sm text-gray-500 placeholder-gray-400 bg-transparent" value={form.endereco} onChange={e => handleChange('endereco', e.target.value)} />
  </div>
  <div className="flex flex-col px-4 border-r last:border-none">
    <span className="text-sm font-semibold text-gray-700">Data</span>
    <input placeholder="Para quando será" className="outline-none text-sm text-gray-500 placeholder-gray-400 bg-transparent" value={form.data} onChange={e => handleChange('data', e.target.value)} />
  </div>
  <div className="flex flex-col px-4">
    <span className="text-sm font-semibold text-gray-700">Valor Máximo</span>
    <input placeholder="Adicionar Valor" className="outline-none text-sm text-gray-500 placeholder-gray-400 bg-transparent" value={form.valor} onChange={e => handleChange('valor', e.target.value)} />
  </div>
  <button className="ml-4 shrink-0 w-10 h-10 bg-blue-700 text-white rounded-full flex items-center justify-center text-lg" onClick={handleSearch}>
    →
  </button>
</div>
        </div>
      ...
      <section className="px-6 mt-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {destaqueItems.map((item, idx) => (
            <div key={idx} className="rounded-lg overflow-hidden shadow-md  bg-[url(/img/mountains.jpg)]" 
            style={{
      ...
      <section className="px-6 mt-10">
        ...
        <div className="flex overflow-x-auto gap-4 pb-4">
          {veiculos.map((v, idx) => (
            <div key={idx} className="min-w-[200px] rounded-lg border p-4 flex-shrink-0 bg-white shadow-sm">
              ...
              <div className="text-xs text-gray-500">Dimensões: {v.dimensao}</div>
              <div className="text-sm font-semibold mt-2 text-blue-700">A partir de R$/frete</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 mt-10">
        <div className="bg-blue-50 border border-blue-200 p-6 rounded-xl flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="text-center md:text-left">
            ...
            <p className="text-sm">Gostaria de anunciar seus serviços em nosso site? Junte-se à nossa plataforma e anuncie o quanto antes!</p>
          </div>
          
          <button className="bg-blue-700 text-white px-4 py-2 rounded-full text-sm">Cadastro de Freteiro</button>
        </div>
